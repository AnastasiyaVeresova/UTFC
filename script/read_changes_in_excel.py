...
columns_mapping = {
    ...
    'Unnamed: 36': ('package_width', None, None),   # Ширина упаковки
    'Unnamed: 37': ('package_depth', None, None),   # Глубина упаковки
    'Unnamed: 38': ('package_height', None, None),   # Высота упаковки
...
for i, model in enumerate(models_excel):
    ...
    model_data["additional_info"]["package_dimensions"] = {
        "width": normalize_value(df.iloc[i + 3, df.columns.get_loc('Unnamed: 36')]),   # Ширина
        "depth": normalize_value(df.iloc[i + 3, df.columns.get_loc('Unnamed: 37')]),  # Глубина
        "height": normalize_value(df.iloc[i + 3, df.columns.get_loc('Unnamed: 38')]) # Высота
    }
...
with open('discrepancies.txt', 'w', encoding='utf-8') as out:
    ...
    for normalized, json_info in models_json.items():
                ...
                for subkey in set(json_val) | set(excel_val):
                    ...
                    if json_subval != excel_subval:
                        discrepancies.append({
                            "Модель": excel_model,
                            "Параметр": f"{key}.{subkey}",
                            "JSON": format_value(json_subval),
                            "Excel": format_value(excel_subval)
                        })
        ...
        # Сравнение additional_info (упаковка)
        ...
        if json_brutto != excel_dims_block['brutto']:
            discrepancies.append({
                "Модель": excel_model,
                "Параметр": "brutto",
                "JSON": format_value(json_dims_block.get('brutto')),
                "Excel": format_value(excel_data[excel_model]['dimensions_details'].get('brutto'))
            })
        if json_netto != excel_dims_block['netto']:
            discrepancies.append({
                "Модель": excel_model,
                "Параметр": "netto",
                "JSON": format_value(json_dims_block.get('netto')),
                "Excel": format_value(excel_data[excel_model]['dimensions_details'].get('netto'))
            })
        if json_volume != excel_dims_block['volume']:
            discrepancies.append({
                "Модель": excel_model,
                "Параметр": "volume",
                "JSON": format_value(json_dims_block.get('volume')),
                "Excel": format_value(excel_data[excel_model]['additional_info'].get('volume'))
            })
